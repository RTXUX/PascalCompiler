namespace PascalCompiler.Lexical.Lexer {
    ...
    /// <summary>
    /// Contains the lexical state IDs for the <c>Pascal</c> language.
    /// </summary>
    /// <remarks>
    ...
    public partial class PascalLexicalStateId : LexicalStateIdProviderBase {
        ...
        private static FieldInfo[] GetFields() {
			#if WINRT
			return typeof(PascalLexicalStateId).GetTypeInfo().DeclaredFields.Where(f => (f.IsPublic) && (f.IsStatic)).ToArray();
			#else
			return typeof(PascalLexicalStateId).GetFields((BindingFlags.Public | BindingFlags.Static));
			#endif
        }
